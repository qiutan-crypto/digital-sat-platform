...
def preprocess_test_markdown(content):
    """
    Fix known OCR issues in Test 7 markdown:
    ...
    content = re.sub(
        r'(Each question has a single best answer\.)\s*\n\n(On the basis of extensive calculations)',
        r'\1\n\n## 1\n\n\2',
        content,
        count=1
    )
    
    # Q15 is missing: after Q14 choices and before the Q15 passage/question
    content = re.sub(
        r'(D\) The time references of noon \(12 p\.m\.\) and midnight \(12 a\.m\.\) are used with roughly the same frequency in the novels\.\s*\n\n<div[^>]*>.*?</div>)\s*\n\n(Percentage of ULE Attributed to Population Growth)',
        r'\1\n\n## 15\n\n\2',
        content,
        count=1,
        flags=re.DOTALL
    ...
    content = re.sub(
        r'(Each question has a single best answer\.)\s*\n\n(Taking photographs in the mid-1800s)',
        r'\1\n\n## 1\n\n\2',
        content,
        count=1
    )
    ...
    if len(q27_positions) >= 1:
        ...
        suffix_m1 = re.sub(
            r"(Don't include symbols such as a percent sign, comma, or dollar sign in your circled answer\.)\s*\n\n(The scatterplot shows the temperature)",
            r"\1\n\n## 1\n\n\2",
            suffix_m1,
            count=1
        )
...
def main():
    test_path = "/Users/lifengliu/Downloads/full-length-sat-paper-practice-test_-bundle-7/sat-practice-test-7-digital-2026-05-03_03-38-00.md"
    answers_path = "/Users/lifengliu/Downloads/full-length-sat-paper-practice-test_-bundle-7/sat-practice-test-7-answers-digital-2026-05-03_03-32-28.md"
    
    print("=" * 60)
    print("=== Parsing Test 7 questions ===")
    print("=" * 60)
    test_modules = parse_test(test_path)
    
    print("\n" + "=" * 60)
    print("=== Parsing Test 7 answers ===")
    print("=" * 60)
    ...
    out_path = "/Users/lifengliu/Documents/liulifeng_文档/python练习文件/SATWEB/data/test7_parsed.json"
